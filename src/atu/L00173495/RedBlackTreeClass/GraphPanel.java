...
/**
 * This code is based on a class by John B. Matthews. See
 * https://sites.google.com/site/drjohnbmatthews/graphpanel NOTE: This class
 * allows for a GUI presentation of the RedBlackTree class When the user inserts
 * a Node it updates accordingly by either rebalancing or recolouring the tree
 * 
 */
public class GraphPanel extends JComponent {
	...
	/**
	 * 
	 * @author Dermot.Hegarty Action class for Insert button Use the insert() method
	 *         (which, in turn, will call handleRedBlack()) to perform the
	 *         insertion.
	 *
	 */
	...
	/**
	 * A DisplayNode represents a node in a graph. This was originally called Node
	 * but I changed it to avoid confusion with the Node class in the Tree
	 */
	private static class DisplayNode {
		...
		/**
		 * Construct a new node. This came with the code and had no value/key
		 */
		...
		public void draw(Graphics g) {
			...
			if (hasNodeValue) {
				...
				g.drawString(nodeValue.toString(), p.x - NodeConstants.X_CHAROFFSET * numDigits,
						p.y + NodeConstants.Y_CHAROFFSET);
			}
			if (selected) {
				g.setColor(Color.darkGray);
	...
	public class BinarySearchTreeViewCapable<T extends Comparable<T>> extends RedBlackTree<T> {
		 ...
		 * the node. It creates a data structure necessary to display the node. It also
		 * creates an edge using the current node and the parent node.
		 * 
		 * @param node
		 * @param depth
		 */
		...
		/*
		 * @Override public void inOrderTraversal() {
		 * 
		 * }
		 * 
		 * @Override protected void recInOrderTraversal(Node subTreeRoot) {
		 * if(subTreeRoot == null) return;
		 * 
		 * recInOrderTraversal(subTreeRoot.left); processNode(subTreeRoot);
		 * //subTreeRoot.xpos = totalNodes++; //subTreeRoot.ypos = depth;
	 ...
	 * 
	 * @Override public int compareTo(java.lang.Integer arg0) { // TODO
